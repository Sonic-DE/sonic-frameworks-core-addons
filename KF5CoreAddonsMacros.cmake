...
# Since 5.88 the macro supports static plugins by passing in the STATIC option.
#
# Example:
...
function(kcoreaddons_add_plugin plugin)
    ...
    if (KCA_ADD_PLUGIN_STATIC)
        ...
        set_property(TARGET ${plugin} PROPERTY AUTOMOC_MOC_OPTIONS "-MX-KDE-PluginNamespace=${KCA_ADD_PLUGIN_INSTALL_NAMESPACE} -MX-KDE-TargetName=${plugin}")
        set_property(TARGET ${plugin} PROPERTY AUTOMOC_MOC_OPTIONS -MX-KDE-PluginNamespace=${KCA_ADD_PLUGIN_INSTALL_NAMESPACE} -MX-KDE-TargetName=${plugin})
...
# This macro imports the plugins frot the given namespacethat were 
# registered using the  kcoreaddons_add_plugin function.
# This includes the Q_IMPORT_PLUGIN statements and linking the plugins to the given target
# Since 5.88 the macro supports static plugins by passing in the STATIC option.
function(kcoreaddons_target_static_plugins app_target plugin_namespace)
    cmake_parse_arguments(ARGS "" "LINK_OPTION" "" ${ARGN})
    set(IMPORT_PLUGIN_STATEMENTS "#include <QPluginLoader>\n\n")
    string(REPLACE "/" "_" plugin_namespace ${plugin_namespace})
    set(TMP_PLUGIN_FILE "${CMAKE_CURRENT_BINARY_DIR}/kcoreaddons_static_${plugin_namespace}_plugins_tmp.cpp")
    set(PLUGIN_FILE "${CMAKE_CURRENT_BINARY_DIR}/kcoreaddons_static_${plugin_namespace}_plugins.cpp")

    foreach(PLUGIN_TARGET_NAME IN LISTS KCOREADDONS_STATIC_PLUGINS${plugin_namespace})
        if (PLUGIN_TARGET_NAME)
            set(IMPORT_PLUGIN_STATEMENTS "${IMPORT_PLUGIN_STATEMENTS}Q_IMPORT_PLUGIN(${PLUGIN_TARGET_NAME}_factory)\;\n")
