...
void FilesystemMetaDataWriterTest::testMissingPermision()
{
    ...
    FilesystemMetaData md(testOutputPath(TEST_FILENAME));
...
void FilesystemMetaDataWriterTest::test()
{
    ...
    FilesystemMetaData md(testFile);
    QVERIFY(md.isSupported());
    ...
    expected = QStringList{QStringLiteral("test.attribute2"), QStringLiteral("test.attribute")};
    QCOMPARE(md.attributes(), expected);
...
{
    FilesystemMetaData md(testOutputPath(TEST_SYMLINK));
