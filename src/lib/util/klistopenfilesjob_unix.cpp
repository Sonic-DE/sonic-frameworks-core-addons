...
private:
    ...
    void lsofFinished(int, QProcess::ExitStatus)
    {
        ...
        auto allProcesses = KProcessList::processInfoList();
        auto findInfoForPid = [&allProcesses](qint64 pid) {
            auto it = std::find_if(allProcesses.begin(), allProcesses.end(), [pid](const KProcessList::KProcessInfo &info) {
                return info.pid() == pid;
            });
            return it != allProcesses.end() ? *it : KProcessList::KProcessInfo{};
        };
#else
