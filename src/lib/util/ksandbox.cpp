...
void KSandbox::startHostProcess(QProcess &process, QProcess::OpenMode mode)
{
    if (!KSandbox::isFlatpak()) {
        return process.start(mode);
    }
    ...
    return process.start(QStringLiteral("/usr/bin/flatpak-spawn"), args, mode);
}
