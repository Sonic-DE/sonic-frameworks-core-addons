...
public:
    ...
    bool emptyMetadataAllowed = false;
...
{
    d->emptyMetadataAllowed = option == AllowEmptyMetaData;
    if (file.endsWith(QLatin1String(".desktop"))) {
        Q_ASSERT_X(!d->emptyMetadataAllowed, Q_FUNC_INFO, "The AllowEmptyMetaData flag is only allowed for binary plugins");
        ...
        if (!qtMetaData.isEmpty()) {
            ...
            if (m_metaData.isEmpty() && !d->emptyMetadataAllowed) {
...
bool KPluginMetaData::isValid() const
{
    ...
    return !pluginId().isEmpty() && (!m_metaData.isEmpty() || d->emptyMetadataAllowed);
}
