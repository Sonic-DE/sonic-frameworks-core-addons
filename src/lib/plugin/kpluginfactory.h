...
public:
    ...
    enum KPluginLoadingErrorReason { NO_ERROR = 0, INVALID_PLUGIN, INVALID_FACTORY, INVALID_KPLUGINFACTORY_INSTANTIATION };
    /**
     * Struct which provides information about errors occurs during plugin loading
     * @since 5.85
     */
    template<typename T>
    class KPluginLoadingResult
    {
    public:
        T *plugin = nullptr;
        ...
        QString errorText;
        KPluginLoadingErrorReason errorReason = NO_ERROR;
     ...
     * @return KPluginLoadingResult object which contains the plugin instance and potentially error information
     * @since 5.85
     */
    static KPluginLoadingResult<KPluginFactory> loadFactory(const KPluginMetaData &data);

    /**
     * Attempts to load the KPluginFactory and create a @p T instance from the given metadata
     * @code
        KPluginFactory::KPluginLoadingResult<MyClass> pluginResult = KPluginFactory::instantiatePlugin<MyClass>(metaData, parent, args);
        if (pluginResult) {
     ...
     */
    template<typename T>
    static KPluginLoadingResult<T> instantiatePlugin(const KPluginMetaData &data, QObject *parent = nullptr, const QVariantList &args = {})
    {
        KPluginLoadingResult<T> result;
        KPluginFactory::KPluginLoadingResult<KPluginFactory> factoryResult = loadFactory(data);
