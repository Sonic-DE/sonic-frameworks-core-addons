...
public:
    ...
    /*!
     * \fn template<typename T> static KPluginFactory::Result<T> KPluginFactory::instantiatePlugin(const KPluginMetaData &data, QObject *parent, const
     * QVariantList &args)
     * TODO qdoc doesn't work
     *
     * Attempts to load the KPluginFactory and create a \a T instance from the given metadata
     ...
     * \a T. If it has multiple choices it's not defined which object will be returned, so be careful
    ...
    /*!
     * TODO qdoc doesn't work
     * Use this method to create an object. It will try to create an object which inherits \a T
