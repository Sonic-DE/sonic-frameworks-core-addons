...
{
    const QString fullADSName = path + QLatin1String(":") + name;
    ...
    LARGE_INTEGER lsize;
    BOOL ret = GetFileSizeEx(hFile.get(), &lsize);
    ...
    ret = ::ReadFile(hFile.get(), data.data(), data.size(), &r, NULL);
...
inline int k_setxattr(const QString &path, const QString &name, const QString &value)
{
    ...
    if (!::WriteFile(hFile.get(), v.constData(), v.size(), &count, NULL)) {
        DWORD error = ::GetLastError();
        std::string message = std::system_category().message(error);
...
{
    QString fullpath = path + QLatin1String(":") + name;
    unique_file_t hFile(::CreateFileW(reinterpret_cast<const WCHAR *>(fullpath.utf16()),
                                      GENERIC_READ,
...
QStringList k_queryAttributes(QStringView path)
{
    ...
    unique_file_t hFile(::CreateFile(reinterpret_cast<const WCHAR *>(path.utf16()),
                                     GENERIC_READ,
                                     FILE_SHARE_READ | FILE_SHARE_WRITE,
                                     NULL,
                                     OPEN_EXISTING,
    ...
    if (::GetFileInformationByHandleEx(hFile.get(), FileStreamInfo, fi, 256 * sizeof(FILE_STREAM_INFO))) {
        ...
        if (error != ERROR_HANDLE_EOF) {
            std::string message = std::system_category().message(error);
            qCWarning(KCOREADDONS_DEBUG) << "failed GetFileInformationByHandleEx:" << message << path << hFile.get();
        }
    }
    delete[] fi;
