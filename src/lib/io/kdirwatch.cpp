...
void KDirWatchPrivate::inotifyEventReceived()
{
    ...
    while (pending > 0) {
        ...
        while (bytesAvailable >= int(sizeof(struct inotify_event))) {
            ...
            qCDebug(KDIRWATCH).nospace() << "got event " << getInotifyEventName(event) << " for entry " << e->path
...
QString KDirWatchPrivate::getInotifyEventName(const inotify_event *event) const
{
