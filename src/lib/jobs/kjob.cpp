...
{
    Q_D(KJob);
    return d->elapsedAccumulatedTime + (d->elapsedTimer ? d->elapsedTimer->elapsed() : 0);
}
...
bool KJob::suspend()
{
    ...
    if (!d->suspended) {
        if (doSuspend()) {
            d->suspended = true;
            if (d->elapsedTimer) {
                d->elapsedAccumulatedTime += d->elapsedTimer->elapsed();
            }
...
void KJobPrivate::speedTimeout()
{
    ...
    Q_EMIT q->elapsedTime(q, q->elapsedTime());
    Q_EMIT q->speed(q, 0);
    speedTimer->stop();
}
